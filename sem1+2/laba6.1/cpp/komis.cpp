...
istream &readFromFile(istream &in, Komisia &obj)
{
    // Сначала читаем Human часть
    readFromFile(in, static_cast<Human&>(obj));
    
    // Потом Komisia часть
    string komis_name, autobio;
    
    // Читаем название комиссии (одно слово)
    in >> komis_name;
    
    // Для автобиографии читаем остаток строки
    getline(in, autobio);
    if (!autobio.empty() && autobio[0] == ' ') {
        autobio.erase(0, 1); // Убираем первый пробел
    }
...
ostream &writeToFile(ostream& out, const Komisia &obj)
{
    // Сначала пишем Human часть
    writeToFile(out, static_cast<const Human&>(obj));
    
    // Потом Komisia часть
    out << " " << obj.komis_name << " " << obj.autobio;
    return out;
}
