...
// Оператор для файлов
istream& readFromFile(istream &in, Student &obj)
{
    ...
    in >> sr_bal >> stependiya;
    obj.set_sr_bal(sr_bal);
    obj.set_stependiya(stependiya);
...
// Оператор для файлов
ostream& writeToFile(ostream& out, const Student &obj)
{
    // Сначала пишем Human часть
    writeToFile(out, static_cast<const Human&>(obj));
    
    // Потом Student часть
    out << " " << obj.sr_bal << " " << obj.stependiya;
    return out;
}
