...
{
    // делегируем поисковую работу универсальному алгоритму, который возвращает Ochered<T>
    return findWithAllParameters(this->begin(), this->end(), example);
}
...
void Ochered<T>::sortByAge()
{
    ...
    do
    {
        ...
        {
            if (current->value.get_age() > current->next->value.get_age())
            {
