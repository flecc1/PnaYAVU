...
istream &readFromFile(istream &in, Prepod_Komis &obj)
{
    // Сначала читаем Prepod часть
    readFromFile(in, static_cast<Prepod&>(obj));
    
    // Потом Komisia часть
    string komis_name, autobio, spisok_rabot;
    
    // Читаем название комиссии
    in >> komis_name;
    
    // Для автобиографии читаем до конца строки
    getline(in, autobio);
    if (!autobio.empty() && autobio[0] == ' ') {
        autobio.erase(0, 1);
    }
    
    // Для списка работ читаем следующую строку
    getline(in, spisok_rabot);
    if (!spisok_rabot.empty() && spisok_rabot[0] == ' ') {
        spisok_rabot.erase(0, 1);
    }
    
    obj.set_komis_name(komis_name.c_str());
    obj.set_autobio(autobio.c_str());
    obj.set_spisok_rabot(spisok_rabot.c_str());
...
ostream &writeToFile(ostream &out, const Prepod_Komis &obj)
{
    // Сначала пишем Prepod часть
    writeToFile(out, static_cast<const Prepod&>(obj));
    
    // Потом Komisia часть и собственные поля
    out << " " << obj.komis_name << " " << obj.autobio << endl;
    out << obj.spisok_rabot;
