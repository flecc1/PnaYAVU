...
class Human
{
    ...
    // Операторы для консоли
    friend istream &operator>>(istream &in, Human &obj);
    friend ostream &operator<<(ostream &out, const Human &obj);
    
    // Операторы для файлов (без валидации)
    friend istream &readFromFile(istream &in, Human &obj);
    friend ostream &writeToFile(ostream &out, const Human &obj);
