...
void Book::displayAllReviews() const
{
    ...
    for (int i = 0; i < reviews.getSize(); i++)
    {
        ...
        cout << (i + 1) << ". ";
        Review& nonConstReview = const_cast<Review&>(review);
        cout << setw(20) << left << nonConstReview.getReviewerName()
             << setw(12) << left << nonConstReview.getDate()
             << setw(8) << left << nonConstReview.getRating()
             << setw(50) << left << (nonConstReview.getText().length() > 47 ? nonConstReview.getText().substr(0, 47) + "..." : nonConstReview.getText()) << endl;
    }
